...
module.exports = function SocketioProtocol () {
  ...
  this.newConnection = function (socket) {
    ...
    try {
      const connection = new this.context.constructors.ClientConnection(this.protocol, ips, socket.handshake.headers);
      this.context.accessors.router.newConnection(connection);

      this.sockets[connection.id] = socket;
      this.socketId2ConnectionId[socket.id] = connection.id;
      ...
      socket.on('disconnect', () => {
        if (this.socketId2ConnectionId[socket.id]) {
          delete this.sockets[this.socketId2ConnectionId];
          delete this.socketId2ConnectionId[socket.id];
        }
